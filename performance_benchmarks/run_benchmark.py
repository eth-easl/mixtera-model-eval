...
def run_benchmark_on_sgs(config: dict, mode: MachineType) -> dict:
    ...
    command = [
        ...
        f'--nproc_per_node={nprocs}', str(run_script_path), '--config-file', str(bm_config_path)
    ]
    typer.echo(f"Running benchmark with command {command}")
    subprocess.run(command, check=True)
    typer.echo(f"Benchmark done, collecting data")
...
def validate_user_input(mode: MachineType,
    model: ModelType,
):
...
def adjust_base_config(
    ...
    assert len(config["data_stages"]) == 1, f"data stages should only be 1"
    ...
    additional_info = {"total_tokens": total_tokens, "tokens_per_step": tokens_per_step, "batch_size": batch_size, "skipped": False, "skip_reason": ""}
...
def run_benchmarks(
    ...
    for (seed, dl_worker, dp, seq_len) in itertools.product(seeds, dl_workers, dps, seq_lengths):
        adjusted_config, additional_info = adjust_base_config(base_config, dataset_path, bm_identifier, curr_run, mode, model, dl_worker, dp, seq_len, seed)
